...
function AppDetailPage() {
  ...
  const updateMetadataMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({
        queryKey: trpc.apps.get.queryOptions({ id: appId }).queryKey,
      });
    },
    onError: (err: any) => {
      addToast(`Failed to update metadata: ${err.message}`, "error");
    },
  });
            ...
            >
              <Input
                ...
                onChange={(e) => {
                  ...
                  setEditedName(next);
                  setMetadataModified(
                    next !== app.metadata.name ||
                      editedDescription !== (app.metadata.description || "") ||
                      editedIcon !== (app.metadata.icon || "") ||
                      editedUrl !== (app.metadata.url || ""),
                  );
                }}
                required
              />
              <Input
                ...
                onChange={(e) => {
                  ...
                  setMetadataModified(
                    editedName !== app.metadata.name ||
                      next !== (app.metadata.description || "") ||
                      editedIcon !== (app.metadata.icon || "") ||
                      editedUrl !== (app.metadata.url || ""),
                  );
                }}
      ...
      <PullProgress
        ...
        onComplete={(result) => {
          ...
          if (result.ok) {
            ...
            addToast(
              `Failed to pull images: ${result.error || "Pull failed"}`,
              "error",
            );
          }
        }}
